...
func TestProviderForName(t *testing.T) {
	if v, ok := providerForName("").(*VMwareOVFProvider); !ok {
		t.Fatalf("bad: %#v", v)
	}

	if v, ok := providerForName("vcenter").(*VMwarevCenterProvider); !ok {
		t.Fatalf("bad: %#v", v)
	}

	if v, ok := providerForName("vcloud").(*VMwarevCloudProvider); !ok {
		t.Fatalf("bad: %#v", v)
	}
